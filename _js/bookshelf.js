...
var currentDemo,
  ...
  samples = {
    ...
    'magazine1' : {'path' : 'images/magazine/'},
...
bookshelf = {
  ...
  showSample: function() {
    ...
    if (Modernizr.csstransforms) {
      ...
      $('.magaz').addClass('preview show-samples show-bar');
      sample.flipbook.turn('page', sample.startPage || 2);
      if (typeof(_gaq)!='undefined')
        _gaq.push(['_trackEvent', 'Sample', currentDemo]);
  ...
  close: function() {
    ...
    if (Modernizr.csstransforms) {
      ...
      $('.magaz').removeClass('no-transition');
      $('.bookshelf').addClass('no-transition no-transform');
      $('.magaz').removeClass('show-bar');
      ...
      setTimeout(function(){
        if (!currentDemo) {
          $('.magaz').removeClass('show-samples sample-'+smpl);
          thumbnail.css({visibility: 'visible'});
        }
      }, 1000);

    } else {
      $('.magaz').removeClass('preview show-samples show-bar sample-'+smpl);
    }
...
function magazHeight() {
  return ($('.magaz').hasClass('preview')) ? 800 : 700;
}
  ...
  Hash.on(sections, {
    yep: function(path, parts) {
      var section = parts[1],
        ...
        magazOffTop = (magazHeight()+100)/2 - parseInt(content.css('marginTop'), 10);

      $('nav a[href!="#'+section+'"]').removeClass('on');
      $('nav a[href="#'+section+'"]').addClass('on');

      if ($.isTouch || $('.magaz').height()>=$(window).height()) {
        scrollTop(content.offset().top-50);
      } else {
        scrollTop(content.offset().top - $('.magaz').height() + magazOffTop, 'fast');
      }
