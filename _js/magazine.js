...
function loadFlipbook(flipbook) {
	...
	flipbook.turn({
		...
		turning: function(e, page, view) {
			...
			if (!$('.magaz .bookshelf').is(':visible'))
				Hash.go('number/' + sampleName+'/'+page).update();
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.magaz').addClass('no-transition').height('');
					$('body > :not(.magaz)').show();
					...
					$('.magaz').addClass('no-transition').height($(window).height());
					$('body > :not(.magaz)').hide();
	...
	if ($.isTouch)
		$('.magaz').bind('zoom.doubleTap', zoomTo);
	else
		$('.magaz').bind('zoom.tap', zoomTo);
